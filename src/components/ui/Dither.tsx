...
class RetroEffectImpl extends Effect {
  ...
  get colorNum(): number {
    return (this.uniforms.get('colorNum')?.value as number) ?? 4.0;
  }
  ...
  get pixelSize(): number {
    return (this.uniforms.get('pixelSize')?.value as number) ?? 2.0;
  }
}
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<{ [uniform: string]: THREE.IUniform }>({
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current.every((v, i) => v === waveColor[i as 0 | 1 | 2])) {
